...
#SBATCH -o /nfs/scratch/papayv/Tarakihi/TARdn/07_MASURCA/RUN_1/job_out_run1
#SBATCH -e /nfs/scratch/papayv/Tarakihi/TARdn/07_MASURCA/RUN_1/job_err_run1
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=yvan.papa@vuw.ac.nz

# assemble.sh generated by masurca
CONFIG_PATH="/nfs/scratch/papayv/Tarakihi/TARdn/07_MASURCA/RUN_1/sr_config_run1.txt"
CMD_PATH="/home/software/apps/masurca/3.2.9/bin/masurca"
set -o pipefail
...
#}
export PATH="/home/software/apps/masurca/3.2.9/bin/../CA/Linux-amd64/bin:/home/software/apps/masurca/3.2.9/bin:$PATH"
save PATH
export PERL5LIB=/home/software/apps/masurca/3.2.9/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=32
save NUM_THREADS
...
rename_filter_fastq 'pe' <(exec expand_fastq '/nfs/scratch/papayv/Tarakihi/TARdn/07_MASURCA/seq_input/TARdn_clean_illumina_R1.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq '/nfs/scratch/papayv/Tarakihi/TARdn/07_MASURCA/seq_input/TARdn_clean_illumina_R2.fastq' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'pe.renamed.fastq'
...
quorum_error_correct_reads  -q $((MIN_Q_CHAR + 40)) --contaminant=/home/software/apps/masurca/3.2.9/bin/../share/adapter.jf -m 1 -s 1 -g 1 -a 3 -t 32 -w 10 -e 3 -M  quorum_mer_db.jf pe.renamed.fastq --no-discard -o pe.cor.tmp --verbose 1>quorum.err 2>&1 && mv pe.cor.tmp.fa pe.cor.fa || fail Error correction of PE reads failed. Check pe.cor.log.
...
/home/software/apps/masurca/3.2.9/bin/mega_reads_assemble_cluster.sh  -Pb 300000000 -q all.q -G 0 -C 25 -t 32 -e $ESTIMATED_GENOME_SIZE -m work1 -n /nfs/scratch/papayv/Tarakihi/TARdn/07_MASURCA/seq_input/TARdn_filtered_nanopore.fa -a /home/software/apps/masurca/3.2.9/bin/../CA8/Linux-amd64/bin -o "   cgwErrorRate=0.15" -B 15 -D 0.02
...
if [ -s $CA_DIR/9-terminator/genome.scf.fasta ];then
  ...
  if [ $NCTG -eq $NSCF ];then
    ...
    if [ -s $CA_DIR/10-gapclose/genome.scf.fasta ];then
      ...
      log 'Gap closing'
      closeGapsLocally.perl --max-reads-in-memory 1000000000 -s 20000000000 --Celera-terminator-directory $CA_DIR/9-terminator --reads-file 'pe.renamed.fastq' --output-directory $CA_DIR/10-gapclose --min-kmer-len 17 --max-kmer-len $(($PE_AVG_READ_LENGTH-5)) --num-threads 32 --contig-length-for-joining $(($PE_AVG_READ_LENGTH-1)) --contig-length-for-fishing 200 --reduce-read-set-kmer-size 21 1>gapClose.err 2>&1
