...
setwd(glue("G:/DATA/WORK/WGS_PIPELINE/2_popgen/16_SNP_analysis_DARTR/210519_3_ADAPTVE_DATASET/q_{qval}_He_{He}"))
dir.create("./results")
vcf_file <- glue("call_set_AGRFlib1_to_6_V2P.rn.QCmaf.AB.adapt_q_{qval}_He_{He}.vcf.gz")
...
pwfst_nosmall_1000 <-stamppFst(glf_nosmall, nboots=1000, percent=95, nclusters=1) #~1h
write.table(pwfst_nosmall_1000$Fsts,file="./results/pwfst_nosmall_1000_Fst.txt")
write.table(pwfst_nosmall_1000$Pvalues,file="./results/pwfst_nosmall_1000_pvalues.txt")

#### 9. DAPC not pre-defined populations ####
print("DAPC")
find.clusters(glf_nosmall,max.n.clust=14, 
n.pca=150, n.clust=6)->clusters
...
dev.off()
pdf(glue("./results/dapc_clusters.pdf"), width = 7, height = 4)
table.value(table(pop(glf_nosmall), clusters$grp), col.lab=paste("inf", 1:15),
            row.lab=rownames(table(pop(glf_nosmall), clusters$grp)))
dev.off()
...
#####10. remove TAS #######
print("10. remove TAS ")
glf_noTAS<- gl.drop.pop(glf_grouped,
                       pop.list=c("04.TBGBJ","07.FRDL","16.CHAT","GBK","18.KTAR","17.TAS"),
                       recalc=T,mono.rm=T,verbose=5)
#165 pops left
print(glf_noTAS)
print(pop(glf_noTAS))
print(levels(pop(glf_noTAS)))

#####10.1 perform PCA#######
print("10.1 perform PCA")
pca_noTAS <- gl.pcoa(glf_noTAS, nfactors=5) #~1h
dir.create("./results/noTAS/")
...
#### if you want to plot PCAs with adegenet ####
##axes1_2
#gl.pcoa.plot(pca_noTAS, glf_noTAS, labels="pop", xaxis=1, yaxis=2)
##dev.print(pdf, paste0('pc1gr_a12.pdf'))
##dev.print(png, paste0('pc1gr_a12.png'),width = 1024, height = 768)
###ind labels
#gl.pcoa.plot(pca_noTAS, glf_noTAS, labels="ind", xaxis=1, yaxis=2)
##dev.print(pdf, paste0('pc1gr_a12_ind.pdf'))
##dev.print(png, paste0('pc1gr_a12_ind.png'),width = 1024, height = 768)
###ellipses
#gl.pcoa.plot(pca_noTAS, glf_noTAS, labels="pop", xaxis=1, yaxis=2,ellipse=T)
##dev.print(pdf, paste0('pc1gr_a12.pdf'))
##dev.print(png, paste0('pc1gr_a12_ellipse.png'),width = 1024, height = 768)
###axes3_4
###pop
#gl.pcoa.plot(pca_noTAS, glf_noTAS, labels="pop", xaxis=3, yaxis=4)
#dev.print(pdf, paste0('pc1_a34.pdf'))
##dev.print(png, paste0('pc1gr_a34.png'),width = 1024, height = 768)
###ellipse
#gl.pcoa.plot(pca_nosmall, glf_noTAS, labels="pop", xaxis=3, yaxis=4,ellipse=T)
#dev.print(pdf, paste0('pc1_a34_ellipse.pdf'))
##dev.print(png, paste0('pc1gr_a34_ellipse.png'),width = 1024, height = 768)
...
pwfst_noTAS_1000 <-stamppFst(glf_noTAS, nboots=100, percent=95, nclusters=1) #~1h
write.table(pwfst_noTAS_1000$Fsts,file="./results/noTAS/pwfst_noTAS_1000Fst.txt")
write.table(pwfst_noTAS_1000$Pvalues,file="./results/noTAS/pwfst_noTAS_1000pvalues.txt")

#### 10.4. DAPC not pre-defined populations ####
print("DAPC")
find.clusters(glf_noTAS,max.n.clust=14, 
              n.pca=150, n.clust=5)->clusters_noTAS #>1h

#Keep all the PCs, there is no downside: (200 to put more)I chose 150)
#we chose 15 clusters but could be more

print(table(pop(glf_noTAS),clusters_noTAS$grp))
png(glue("./results/noTAS/dapc_clusters.png"), width = 6, height = 4, units = "in",res=300)
table.value(table(pop(glf_noTAS),clusters_noTAS$grp), col.lab=paste("inf", 1:15),
            row.lab=rownames(table(pop(glf_noTAS), clusters_noTAS$grp)))
dev.off()

dapc_nt <- dapc(glf_noTAS, clusters_noTAS$grp,
n.pca=50, n.da=4) #~1h
#We want to retain a minimum of PCs: keep 50
#Keep 14 eigenvalues
### plot DAPC 1:2 ###
