...
#SBATCH --time=2:00:00
#SBATCH -o /nfs/scratch/papayv/Tarakihi/popgen/13_filters/lib1_to_6_V2P/filter3/jobs_reports/%A_%a_AB_and_neutral_filtering.out
#SBATCH -e /nfs/scratch/papayv/Tarakihi/popgen/13_filters/lib1_to_6_V2P/filter3/jobs_reports/%A_%a_AB_and_neutral_filtering.err
...
vcftools 	--gzvcf $outdir_AB/$name.scf$scf.AB.vcf.gz --out $outdir_pruned/$name.scf$scf.AB.hwe_thin \
			--hwe 0.05 --maf 0.01 --thin 1500  --recode-INFO-all --recode
mv $outdir_pruned/$name.scf$scf.AB.hwe_thin.recode.vcf $outdir_pruned/$name.scf$scf.AB.hwe_thin.vcf
bcftools stats --depth 1,100,1 $outdir_pruned/$name.scf$scf.AB.hwe_thin.vcf > $outdir_pruned/$name.scf$scf.AB.hwe_thin.stats.txt
bgzip -f -i $outdir_pruned/$name.scf$scf.AB.hwe_thin.vcf

echo "convert vcf to tped file"
vcftools 	--gzvcf $outdir_pruned/$name.scf$scf.AB.hwe_thin.vcf.gz --out $outdir_pruned/$name.scf$scf.AB.hwe_thin --plink-tped
#sed -i 's/^0\tLG\([0-9]*\)/\1\t\1/g' $outdir_pruned/$name.scf$scf.AB.hwe_thin.tped 

echo "prune file for pairwise Linkage Desequilibrium"
plink 	--tped $outdir_pruned/$name.scf$scf.AB.hwe_thin.tped --tfam $outdir_pruned/$name.scf$scf.AB.hwe_thin.tfam 				\
		--indep-pairwise 50 5 0.2 --out $outdir_pruned/$name.scf$scf.AB.hwe_thin --allow-extra-chr #--chr-set 24 no-xy no-mt
sed 's/^\([0-9]*\):/\1\t/g' $outdir_pruned/$name.scf$scf.AB.hwe_thin.prune.in > $outdir_pruned/$name.scf$scf.AB.hwe_thin.prune.in.list

echo "fitler retained SNPs"
vcftools 	--gzvcf $outdir_pruned/$name.scf$scf.AB.hwe_thin.vcf.gz --out $outdir_pruned/$name.scf$scf.AB.hwe_thin.pruned 		\
			--positions $outdir_pruned/$name.scf$scf.AB.hwe_thin.prune.in.list --recode-INFO-all --recode
			
mv $outdir_pruned/$name.scf$scf.AB.hwe_thin.pruned.recode.vcf $outdir_pruned/$name.scf$scf.AB.hwe_thin.pruned.vcf
bcftools stats --depth 1,100,1 $outdir_pruned/$name.scf$scf.AB.hwe_thin.pruned.vcf > $outdir_pruned/$name.scf$scf.AB.hwe_thin.pruned.stats.txt
bgzip -f -i $outdir_pruned/$name.scf$scf.AB.hwe_thin.pruned.vcf 
