#!/bin/bash
#SBATCH --job-name=busco_gene_models_BAR
#SBATCH --cpus-per-task=32
#SBATCH --mem=64G
#SBATCH --partition=parallel
#SBATCH --time=1-10:00:00
#SBATCH -o /nfs/scratch/papayv/Tarakihi/TARdn/Z_fish_assemblies/10_genome_annotation/BAR/3_model_evaluations/busco_genemodels_job_%A_%a.out
#SBATCH -e /nfs/scratch/papayv/Tarakihi/TARdn/Z_fish_assemblies/10_genome_annotation/BAR/3_model_evaluations/busco_genemodels_job_%A_%a.err
...
time python $busco_scripts_path/run_BUSCO.py -i $dir/BARdn_round1.all.maker.proteins.fasta -o busco_prots_round1 -l $lineage \
  -m proteins -c $threads -sp Butterfish -z --augustus_parameters='--progress=true'
  
echo "transcripts round1"
time python $busco_scripts_path/run_BUSCO.py -i $dir/BARdn_round1.all.maker.transcripts.fasta -o busco_mrna_round1 -l $lineage \
  -m transcriptome -c $threads -sp Butterfish -z --augustus_parameters='--progress=true'
  
  echo "proteins round2"
time python $busco_scripts_path/run_BUSCO.py -i $dir/BARdn_round2.all.maker.proteins.fasta -o busco_prots_round2 -l $lineage \
  -m proteins -c $threads -sp Butterfish -z --augustus_parameters='--progress=true'
  
echo "transcripts round2"
time python $busco_scripts_path/run_BUSCO.py -i $dir/BARdn_round2.all.maker.transcripts.fasta -o busco_mrna_round2 -l $lineage \
  -m transcriptome -c $threads -sp Butterfish -z --augustus_parameters='--progress=true'
