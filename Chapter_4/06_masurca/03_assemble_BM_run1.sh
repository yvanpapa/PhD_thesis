#!/bin/bash
#SBATCH --job-name=BM_run1_masurca
#SBATCH --cpus-per-task=32
#SBATCH --mem=600G
#SBATCH --partition=bigmem
#SBATCH --time=10-00:00:00
#SBATCH -o /nfs/scratch/papayv/Tarakihi/TARdn/Z_fish_assemblies/7_masurca/BM/run1/%j_out_run1_BM
#SBATCH -e /nfs/scratch/papayv/Tarakihi/TARdn/Z_fish_assemblies/7_masurca/BM/run1/%j_err_run1_BM
...
CONFIG_PATH="/nfs/scratch/papayv/Tarakihi/TARdn/Z_fish_assemblies/7_masurca/BM/run1/sr_config_runBM.txt"
...
rename_filter_fastq 'pe' <(exec expand_fastq '/nfs/scratch/papayv/Tarakihi/TARdn/Z_fish_assemblies/4b_bwa_mitogenome/005_BM05.mitogenome.unmapped.end1.fq.gz' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}') <(exec expand_fastq '/nfs/scratch/papayv/Tarakihi/TARdn/Z_fish_assemblies/4b_bwa_mitogenome/005_BM05.mitogenome.unmapped.end2.fq.gz' | awk '{if(length($0>250)) print substr($0,1,250); else print $0;}' ) > 'pe.renamed.fastq'
...
if [ -s $CA_DIR/9-terminator/genome.scf.fasta ];then
  ...
  if [ $NCTG -eq $NSCF ];then
    ...
    if [ -s $CA_DIR/10-gapclose/genome.scf.fasta ];then
      ...
      log 'Gap closing'
      closeGapsLocally.perl --max-reads-in-memory 1000000000 -s 10000000000 --Celera-terminator-directory $CA_DIR/9-terminator --reads-file 'pe.renamed.fastq' --output-directory $CA_DIR/10-gapclose --min-kmer-len 17 --max-kmer-len $(($PE_AVG_READ_LENGTH-5)) --num-threads 32 --contig-length-for-joining $(($PE_AVG_READ_LENGTH-1)) --contig-length-for-fishing 200 --reduce-read-set-kmer-size 21 1>gapClose.err 2>&1
