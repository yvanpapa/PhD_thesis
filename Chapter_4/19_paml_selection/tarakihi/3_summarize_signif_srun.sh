...
list_fdr_signif_ortho=/nfs/scratch/papayv/Tarakihi/TARdn/Z_fish_assemblies/24_paml_selection/ppl1_cds_array_tarakihi/fdr_signif_orthogroups.txt
...
#manually remove all the non-significant orthogroups in fdr_signif_orthogroups.txt > final_selected_orthogroups.txt
...
rm $dir/adapted_genes_functions.txt
for f in "${array[@]}" ; do
echo $f >> $dir/adapted_genes_functions.txt
seqkit seq --name $fastadir/$f.fullname.fa >> $dir/adapted_genes_functions.txt
done
